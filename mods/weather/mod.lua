...
weather = {
    ...
    weatherboxes = {},
    weatherbox_radius = tonumber(core.settings:get("neotest_weather_radius") or "18") or 18 
}
...
local function get_height_for_box(pos_2d, base, radius)
    ...
    local sky_obstructed = first_up ~= nil or base >= 120
...
function Weatherbox:update()
    ...
    for dx = min_x, max_x do
        for dz = min_z, max_z do
            local pos_2d = {x=dx, z=dz}
            local pos_2d_hash = hash_pos_2d(pos_2d)
            if not old_spawners[pos_2d_hash] then
                local sky_obstructed, max, min = get_height_for_box(pos_2d, base_y, self.radius)
                if not sky_obstructed then
                    local minpos = vector.new(pos_2d.x, min, pos_2d.z)
                    local maxpos = vector.new(pos_2d.x, max, pos_2d.z)
                    local texture, velocity, intensity = get_weather_state(self.pos)
                    local spawner = configure_spawner(minpos, maxpos, texture, velocity, self.playername, intensity)
...
local function set_player(player, weathercode)
...
core.register_on_joinplayer(function(player) 
    set_player(player, weather.current_weather)
    local player_name = player:get_player_name()
    pos = player:get_pos()
    if not weather.weatherboxes[player_name] then

        weather.weatherboxes[player_name] = Weatherbox:new(player_name, pos, weather.weatherbox_radius)
    end
...
function weather.set_weather(weathercode)
    ...
    for i, player in ipairs(all_players) do
        set_player(player, weathercode)
    end
    for _, weatherbox in pairs(weather.weatherboxes) do
        local active = weathercode ~= 0
...
function player_tracker.track()
    ...
    for _, player in ipairs(players) do
        ...
        local last_pos = (player_tracker.players[name] or vector.zero())
...
player_tracker.attach_listener(function(name, pos)
    if weather.weatherboxes[name] then
        local weatherbox = weather.weatherboxes[name]
